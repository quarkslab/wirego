...
class Wirego:
    ...
    def _result_get_protocol(self, socket, messageFrames):
        ...
        packet_number = int.from_bytes(
            messageFrames[1], "little"
        )  # Frame 1 is the dissection handler
    ...
    def _result_get_field(self, socket, messageFrames):
        ...
        socket.send(b"\x01", zmq.SNDMORE)
        socket.send(
            result.fields[idx].parent_idx.to_bytes(4, byteorder="little", signed=True),
            zmq.SNDMORE,
        )
    ...
    def _add_result_to_cache(self, result, pktnum):
        ...
        self.cache[pktnum] = (
            flatten  # Since we have one result per packet number, use pktnum as key
        )

    # _add_fields_recursive recursively adds fields and subfields to cache and compute parent_idx
    def _add_fields_recursive(
        self, flatten: DissectResultFlattenEntry, parent_idx: int, field: DissectField
    ):
        new_parent_idx: int
        field_flatten = DissectResultFieldFlatten(
            parent_idx, field.wirego_field_id, field.offset, field.length
        )
