...
class WiregoMinimal(wirego.WiregoListener):
    ...
    def get_fields(self):
        return [
            wirego.WiregoField(
                FieldEnum.FieldIdCustom1,
                "Custom1",
                "wgminexample.custom01",
                wirego.ValueType.ValueTypeUInt8,
                wirego.DisplayMode.DisplayModeHexadecimal,
            ),
            wirego.WiregoField(
                FieldEnum.FieldIdCustom2,
                "Custom2",
                "wgminexample.custom02",
                wirego.ValueType.ValueTypeUInt16,
                wirego.DisplayMode.DisplayModeDecimal,
            ),
            wirego.WiregoField(
                FieldEnum.FieldIdCustomWithSubFields,
                "Custom With Subs",
                "wgminexample.custom_subs",
                wirego.ValueType.ValueTypeUInt32,
                wirego.DisplayMode.DisplayModeHexadecimal,
            ),
        ]
    ...
    def get_detection_filters(self):
        return [
            wirego.DetectionFilter(
                wirego.DetectionFilterType.DetectionFilterTypeInt, "udp.port", 137, ""
            ),
            wirego.DetectionFilter(
                wirego.DetectionFilterType.DetectionFilterTypeStr,
                "bluetooth.uuid",
                0,
                "1234",
            ),
    ...
    def detection_heuristic(
        self, packet_number: int, src: str, dst: str, stack: str, packet: bytes
    ) -> bool:
    ...
    def dissect_packet(
        ...
        if len(packet) > 10:
            ...
            field = wirego.DissectField(
                FieldEnum.FieldIdCustomWithSubFields, 6, 4, [subField1, subField2]
            )
            fields.append(field)
